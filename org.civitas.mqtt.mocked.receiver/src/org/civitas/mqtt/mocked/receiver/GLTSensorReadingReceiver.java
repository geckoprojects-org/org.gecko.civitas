 ...
 * @since Sep 24, 2025
 */
@Component(name = "GLTSensorReadingReceiver", immediate = true)
public class GLTSensorReadingReceiver {

	private static final Logger LOGGER = Logger.getLogger(GLTSensorReadingReceiver.class.getName());
	...
	@Activate
	public GLTSensorReadingReceiver(
	...
	private void hanldeMessage(Message msg) {
		...
		SensorReading alarm = extractAlarm(msg);
		if(alarm != null) {
//			LOGGER.info(
//					String.format("""
...
//					);
	...
	private SensorReading extractAlarm(Message message) {		
		byte[] content = message.payload().array();
		try {
			return (SensorReading) MqttReceiverHelper
					.loadResource(rsFactory, URI.createFileURI(UUID.randomUUID().toString().concat(".json")), 
							"application/json", content, 
							Collections.singletonMap(EMFJs.OPTION_ROOT_ELEMENT, BuildingSensorPackage.Literals.SENSOR_READING));
		} catch(Exception e) {
